...
class YourTweet extends React.Component{
    ...
    trymakeRequest=async(num)=>{
        try {
            let name = this.props.location.pathname.split('/')
             const res = await axios({url:`https://tweetit-react.herokuapp.com//tweet/${name[1]}/all`, headers:{'Authorization':`Bearer ${this.props.user.token}`}})
             if(res.status==200)
            { 
    ...
    makeRequest=async()=>{
       try {
            const res = await axios({url:`https://tweetit-react.herokuapp.com//tweet/${this.props.user.username}/all`, headers:{'Authorization':`Bearer ${this.props.user.token}`}})
            if(res.status==200)
        {
            await this.setState({data:res.data.tweets[0].tweet})
           await this.setState({loaded:true})
